...
<body>
    <div class="container">
        ...
        <!-- 修改：调整说明部分的措辞，使其更中性专业 -->
        <div class="report-content" style="margin-top: 30px;">
            <h2>📖 图表解读与策略说明</h2>
            <p><strong>重要提示：</strong>以下内容由程序根据预设量化策略自动生成，旨在提供历史数据分析的参考信息，<strong>不构成任何投资建议</strong>。市场存在风险，投资决策需谨慎，请务必基于您个人的判断和风险承受能力。</p>
            ...
            <ul>
                 <li><strong>上图 (价格与信号):</strong>
                    <ul>
                        <li><strong>深蓝色线 (价格):</strong> 代表每日的白银收盘价，是分析的基础。</li>
                        <li><strong>橙色虚线 (短期均线):</strong> 反映近期价格的平均水平。价格在此线上方通常表示短期市场看好，反之则表示短期市场看淡。</li>
                        <li><strong>红色/绿色细线 (EMA):</strong> 指数移动平均线，对近期价格赋予更高权重，辅助判断短期趋势的强度与方向。</li>
                        <li><strong><span style="color:red; font-size: 1.2em;">▲</span> 红色三角 (采购信号):</strong> 策略输出的关键信号。当此标记出现时，表示根据下方图表的综合分析，策略认为当前**可能**存在潜在的买入机会。<strong>但这仅为策略信号，不代表必须买入。</strong></li>
                    </ul>
                </li>
                <li><strong>中图 (策略核心指标):</strong>
                    <ul>
                        <li><strong>蓝色实线 (核心工业指标):</strong> 本策略构建的核心指标，综合考量当前价格相对于历史均值水平的偏离，以及近期市场的波动情况。通常，该指标值较低时，策略倾向于认为价格可能被低估。</li>
                        <li><strong>红色虚线 (中期阈值 - 警戒线):</strong> 策略为"核心指标"设定的动态参考线。当蓝色实线**低于**此红色虚线（进入淡绿色区域）时，视为满足策略的第一个关键买入条件。</li>
                        <li><strong>其他虚线 (短/长期阈值):</strong> 提供不同时间周期下核心指标的相对位置参考。</li>
                        <li><strong>灰色点线 (基准=1):</strong> 指标等于1代表中性状态，低于1通常被策略视为更有利的区域。</li>
                    </ul>
                </li>
                <li><strong>下图 (市场动量指标 - RSI):</strong>
                    <ul>
                        <li><strong>紫色实线 (修正RSI):</strong> 相对强弱指数（RSI），用于衡量市场买卖双方的力量对比。当该指标低于特定水平（如图中红色点线45）时，常被解读为市场可能处于"超卖"状态，短期价格反弹的可能性或会增加。</li>
                        <li><strong>橙色虚线 (动态RSI阈值):</strong> 策略动态计算的RSI参考阈值。</li>
                        <li><strong>红色点线 (超卖参考线=45):</strong> 策略设定的固定参考阈值。当紫色RSI线**低于**此线时，视为满足策略的第二个关键买入条件。</li>
                    </ul>
                </li>
            </ul>
            <h3>策略决策逻辑简介：</h3>
             <p>该策略在生成 <span style="color:red; font-size: 1.2em;">▲</span> 采购信号时遵循相对审慎的原则，需要同时满足以下条件：</p>
            <ol>
                <li><strong>主要条件满足度：</strong>以下六个核心条件中，至少需要满足四项：
                    <ul>
                        <li>① 核心工业指标低于其中期阈值？</li>
                        <li>② 修正RSI低于45？</li>
                        <li>③ 当前价格低于EMA21？</li>
                        <li>④ 当前价格低于或接近布林通道下轨参考值？</li>
                        <li>⑤ 短期EMA比率高于其动态阈值？</li>
                        <li>⑥ 市场动量因子低于其动态阈值（市场相对平静）？</li>
                    </ul>
                </li>
                <li><strong>无"信号阻断"情况：</strong>即使满足了足够的主要条件，若出现以下任一情况，策略也会暂停发出信号：
                    <ul>
                        <li>近期的价格形态显示不利特征（如快速上涨后大幅回落）。</li>
                        <li>价格进入基于ATR计算的波动通道的超买区域。</li>
                        <li>距离上次策略发出信号的时间间隔过短（当前设定为{analysis_data['min_purchase_interval']}天）。</li>
                    </ul>
                </li>
            </ol>
            <p><strong>策略信号生成的条件是：主要条件满足数量达标，并且没有触发任何信号阻断情况。</strong> 策略中的动态窗口调整机制，旨在尝试适应不同市场波动环境。</p>

            <!-- 动态解读部分保持不变，因其措辞已较为客观 -->
