...
def generate_report(df, optimized_quantile, optimized_rsi_threshold):
    ...
    if len(df) >= N_DAYS_RECENT:
        ...
        if not missing_cols_recent:
            ...
            # --- 5. 新增: 低于全部阈值 (Recent) ---
            all_thresh_signal_recent = (df_recent['工业指标'] < df_recent['基线阈值_短']) & \
                                       (df_recent['工业指标'] < df_recent['基线阈值']) & \
                                       (df_recent['工业指标'] < df_recent['基线阈值_长'])
            all_thresh_buys_recent = df_recent[all_thresh_signal_recent]
            all_points_recent = len(all_thresh_buys_recent)
            if all_points_recent > 0:
                avg_all_thresh_cost_recent = safe_float(all_thresh_buys_recent['Price'].mean())
                advantage_rate = ((avg_market_price_recent - avg_all_thresh_cost_recent) / avg_market_price_recent) * 100 if avg_market_price_recent > 0 else 0
                advantage_text = f"<span style='color: {'green' if advantage_rate >= 0 else 'red'};'>{advantage_rate:+.1f}%</span>" if avg_market_price_recent > 0 else "N/A (市场均价为0)"
                avg_interval_recent = total_days_recent / all_points_recent
                # --- 计算最大间隔 ---
                max_interval_recent = 'N/A'
                if all_points_recent > 1:
                    intervals = np.diff(all_thresh_buys_recent.index)
                    if len(intervals) > 0:
                        max_interval_recent = intervals.max()
                avg_interval_recent_text = f"{avg_interval_recent:.1f} / {max_interval_recent}"
                # --- 结束计算最大间隔 ---
                results_recent['低于全部阈值'] = (f"{avg_all_thresh_cost_recent:.2f}", advantage_text, all_points_recent, avg_interval_recent_text)
            else:
    ...
    if not df.empty: # Check if df is not empty first
        ...
        if not missing_cols_analysis:
            ...
            # --- 5. 新增: 低于全部阈值 ---
            all_thresh_signal_full = (df_analysis_scope['工业指标'] < df_analysis_scope['基线阈值_短']) & \
                                     (df_analysis_scope['工业指标'] < df_analysis_scope['基线阈值']) & \
                                     (df_analysis_scope['工业指标'] < df_analysis_scope['基线阈值_长'])
            all_thresh_buys_full = df_analysis_scope[all_thresh_signal_full]
            all_points = len(all_thresh_buys_full)
            if all_points > 0:
                avg_all_thresh_cost = safe_float(all_thresh_buys_full['Price'].mean())
                advantage_rate = ((avg_market_price_full - avg_all_thresh_cost) / avg_market_price_full) * 100 if avg_market_price_full > 0 else 0
                advantage_text = f"<span style='color: {'green' if advantage_rate >= 0 else 'red'};'>{advantage_rate:+.1f}%</span>" if avg_market_price_full > 0 else "N/A (市场均价为0)"
                avg_interval = total_days_in_scope / all_points
                # --- 计算最大间隔 ---
                max_interval = 'N/A'
                if all_points > 1:
                    intervals = np.diff(all_thresh_buys_full.index)
                    if len(intervals) > 0:
                        max_interval = intervals.max()
                avg_interval_text = f"{avg_interval:.1f} / {max_interval}"
                # --- 结束计算最大间隔 ---
                results['低于全部阈值'] = (f"{avg_all_thresh_cost:.2f}", advantage_text, all_points, avg_interval_text)
            else:
...
if __name__ == "__main__":
    ...
    # --- 获取优化参数 (保持不变) ---
    optimized_quantile = 0.15
    optimized_rsi_threshold = 30
    print("\n将使用固定的指定参数生成最终报告：")
